...
Some vendor-native marketplace installers require a public index file at a
fixed repository-root path rather than under `dist/`. In that case, the
generated payload still lives under `dist/<vendor>/marketplace/`, and the
root-level index file may be generated only when all of these are true:

- The vendor installer documents that exact root-relative marketplace path.
- The index points at the selected generated `dist/<vendor>/marketplace/`
  payload or another public vendor-supported source.
- The index is generated by the vendor adapter and checked by its freshness
  validation.
- No root-level generated policy content is introduced outside the vendor index.
